...
const lessonGenBtn = document.getElementById('lessonGenBtn');
const lessonGenModalOverlay = document.getElementById('lessonGenModalOverlay');
const lessonGenModalSub = document.getElementById('lessonGenModalSub');
const lessonGenModalCancelBtn = document.getElementById('lessonGenModalCancelBtn');
const lessonGenOptions = [
  { el: document.getElementById('lessonGenOptUnit'), scopeUnits: 1, label: 'Rest of this unit' },
  { el: document.getElementById('lessonGenOptTwoUnits'), scopeUnits: 2, label: 'This unit + the next one' },
  { el: document.getElementById('lessonGenOptCourse'), scopeUnits: UNITS_PER_COURSE, label: 'Rest of the course' },
];
const lessonGenBar = document.getElementById('lessonGenBar');
const lessonGenBarIcon = document.getElementById('lessonGenBarIcon');
const lessonGenBarTitle = document.getElementById('lessonGenBarTitle');
const lessonGenBarSub = document.getElementById('lessonGenBarSub');
const lessonGenBarFill = document.getElementById('lessonGenBarFill');
const lessonGenBarToggle = document.getElementById('lessonGenBarToggle');
const lessonGenBarToggleIcon = document.getElementById('lessonGenBarToggleIcon');
const lessonGenBarClose = document.getElementById('lessonGenBarClose');
...
const chessLessonsBtn = document.getElementById('chessLessonsBtn');
...
const lessonStartSpinner = document.getElementById('lessonStartSpinner');
const lessonStartRetryBtn = document.getElementById('lessonStartRetryBtn');
...
let lessonStartGateState = 'ready'; // 'loading' | 'failed' | 'ready' — Start is only pressable once the lesson title exists
let lessonGenJob = null;            // the background "Generate Lessons" job, if any (see BACKGROUND LESSON GENERATION)
const lessonGenInFlight = new Map();   // `${courseId}_${unitIndex}_${lessonIndex}` -> promise for a lesson the background job is generating right now
const interactiveGenKeys = new Set();  // same keys, for lessons being generated by the Start button — the background job skips these
...
export function resetLearnState() {
  window.dispatchEvent(new CustomEvent('kll:learn-reset')); // lets the chess module (index.html) drop its in-progress game
  cancelLessonGenJob();
...
// Lets the chess module (index.html) key its saved Game History per account.
export function getLearnUid() {
  return uid();
}
...
// ============================================================
// CHESS DIAGRAM (static) — rebuilt from scratch.
// One SVG coordinate system (viewBox 0 0 100 100 for the 8x8 grid plus a
// label gutter) so squares, pieces, highlights and the arrow can never
// drift out of alignment. White-perspective: a1 bottom-left, h8 top-right.
// Fails closed: anything malformed is skipped, never thrown.
// ============================================================
const CHESS_SQ_RE = /^[a-h][1-8]$/;
const CHESS_LIGHT_SQ = '#F0D9B5';
const CHESS_DARK_SQ = '#B58863';
const CHESS_GUTTER = 5;            // label gutter (viewBox units) on left + bottom
const CHESS_CELL = 11.25;          // 90 / 8

// square name -> top-left corner of that square in viewBox units.
// file a..h -> col 0..7 left-to-right; rank 8..1 -> row 0..7 top-to-bottom.
function chessSquareOrigin(sq) {
  const col = sq.charCodeAt(0) - 97;
  const row = 8 - Number(sq[1]);
  return { x: CHESS_GUTTER + col * CHESS_CELL, y: row * CHESS_CELL };
}
// Colour is derived from the square NAME, not from loop parity.
// file index 0..7 (a..h) + rank 1..8: a1 -> 0+1 = 1 (odd)  => dark.
// So an ODD sum is dark, an EVEN sum is light:
//   a1 dark, h1 (7+1=8) light, a8 (0+8=8) light, h8 (7+8=15) dark.
function chessSquareIsDark(sq) {
  return ((sq.charCodeAt(0) - 97) + Number(sq[1])) % 2 === 1;
}

// Pieces are the same Lichess (cburnett) piece set used in Play mode
// (src/play/vendor/pieces/*.svg — each a 45x45 viewBox, so they drop
// straight into the same 0..45 box the diagram grid already assumes).
const CHESS_PIECE_TYPES = new Set(['P', 'R', 'N', 'B', 'Q', 'K']);
const CHESS_PIECE_FILES = {
  w: { P: 'wP.svg', R: 'wR.svg', N: 'wN.svg', B: 'wB.svg', Q: 'wQ.svg', K: 'wK.svg' },
  b: { P: 'bP.svg', R: 'bR.svg', N: 'bN.svg', B: 'bB.svg', Q: 'bQ.svg', K: 'bK.svg' },
};

function chessPieceSvg(sq, type, color) {
  const file = CHESS_PIECE_FILES[color]?.[type];
  if (!file) return '';
  const { x, y } = chessSquareOrigin(sq);
  const scale = CHESS_CELL / 45;
  return `<g class="chess-diagram-piece chess-diagram-piece-${color}" data-sq="${sq}" transform="translate(${x} ${y}) scale(${scale})"><image href="./play/vendor/pieces/${file}" xlink:href="./play/vendor/pieces/${file}" x="0" y="0" width="45" height="45"/></g>`;
}

function renderChessDiagram(data) {
  const d = data && typeof data === 'object' ? data : {};
  const pieces = Array.isArray(d.pieces) ? d.pieces : [];
  const highlights = Array.isArray(d.highlights) ? d.highlights : [];

  // Squares (row-major from the top-left, i.e. a8 first).
  let squares = '';
  for (let rank = 8; rank >= 1; rank--) {
    for (let f = 0; f < 8; f++) {
      const sq = 'abcdefgh'[f] + rank;
      const { x, y } = chessSquareOrigin(sq);
      squares += `<rect x="${x}" y="${y}" width="${CHESS_CELL}" height="${CHESS_CELL}" fill="${chessSquareIsDark(sq) ? CHESS_DARK_SQ : CHESS_LIGHT_SQ}" data-sq="${sq}"/>`;
    }
  }

  // Highlights: translucent tint drawn UNDER the pieces so a piece on a
  // highlighted square is never obscured.
  let hi = '';
  const seenHi = new Set();
  for (const s of highlights) {
    if (typeof s !== 'string' || !CHESS_SQ_RE.test(s) || seenHi.has(s)) continue;
    seenHi.add(s);
    const { x, y } = chessSquareOrigin(s);
    hi += `<rect class="chess-diagram-hi" x="${x}" y="${y}" width="${CHESS_CELL}" height="${CHESS_CELL}" fill="#F6E94B" fill-opacity="0.6" data-hi="${s}"/>`;
  }

  // Pieces: last entry wins if two land on the same square.
  const bySq = new Map();
  for (const p of pieces) {
    if (!p || typeof p !== 'object') continue;
    const sq = typeof p.square === 'string' ? p.square.trim().toLowerCase() : '';
    const type = typeof p.piece === 'string' ? p.piece.trim().toUpperCase() : '';
    const color = typeof p.color === 'string' ? p.color.trim().toLowerCase() : '';
    if (!CHESS_SQ_RE.test(sq) || !CHESS_PIECE_TYPES.has(type) || (color !== 'w' && color !== 'b')) continue;
    bySq.set(sq, { type, color });
  }
  let pcs = '';
  for (const [sq, p] of bySq) pcs += chessPieceSvg(sq, p.type, p.color);

  // Arrow: shaft from centre of `from` to just short of the centre of `to`,
  // with a triangular head oriented along the true vector (so knight-shaped
  // arrows like g5->f7 point correctly).
  let arrow = '';
  const a = d.arrow;
  if (a && typeof a === 'object' && CHESS_SQ_RE.test(a.from) && CHESS_SQ_RE.test(a.to) && a.from !== a.to) {
    const f = chessSquareOrigin(a.from), t = chessSquareOrigin(a.to);
    const cx1 = f.x + CHESS_CELL / 2, cy1 = f.y + CHESS_CELL / 2;
    const cx2 = t.x + CHESS_CELL / 2, cy2 = t.y + CHESS_CELL / 2;
    const vl = Math.hypot(cx2 - cx1, cy2 - cy1);
    // Pull the tip back ~3 units from the target centre so it lands on the
    // near edge of the square instead of on top of the piece body.
    const x1 = cx1, y1 = cy1;
    const x2 = cx2 - (cx2 - cx1) / vl * 3, y2 = cy2 - (cy2 - cy1) / vl * 3;
    const dx = x2 - x1, dy = y2 - y1;
    const len = Math.hypot(dx, dy);
    const ux = dx / len, uy = dy / len;
    const head = 3.2, half = 1.9;
    const bx = x2 - ux * head, by = y2 - uy * head; // base of head
    const sx = x1 + ux * 2.2, sy = y1 + uy * 2.2;   // start slightly off the piece centre
    const px = -uy, py = ux;                          // perpendicular
    arrow = `<g class="chess-diagram-arrow" opacity="0.8" fill="#3F8F2F" stroke="none">`
      + `<line x1="${sx.toFixed(2)}" y1="${sy.toFixed(2)}" x2="${bx.toFixed(2)}" y2="${by.toFixed(2)}" stroke="#3F8F2F" stroke-width="1.4" stroke-linecap="round"/>`
      + `<polygon points="${x2.toFixed(2)},${y2.toFixed(2)} ${(bx + px * half).toFixed(2)},${(by + py * half).toFixed(2)} ${(bx - px * half).toFixed(2)},${(by - py * half).toFixed(2)}"/></g>`;
  }

  // Labels: files a-h along the bottom, ranks 8-1 down the left.
  let labels = '';
  for (let f = 0; f < 8; f++) {
    labels += `<text x="${CHESS_GUTTER + f * CHESS_CELL + CHESS_CELL / 2}" y="93" text-anchor="middle" class="chess-diagram-label">${'abcdefgh'[f]}</text>`;
  }
  for (let r = 0; r < 8; r++) {
    labels += `<text x="2.2" y="${r * CHESS_CELL + CHESS_CELL / 2 + 1.4}" text-anchor="middle" class="chess-diagram-label">${8 - r}</text>`;
  }

  return `<div class="chess-diagram-wrap"><svg class="chess-diagram" viewBox="0 0 95 94.5" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" role="img" aria-label="Chess diagram">${squares}${hi}${pcs}${arrow}${labels}</svg></div>`;
}
...
// ---- Learn / Play sub-tabs (chess courses only) ----
const learnSubtabs = document.getElementById('learnSubtabs');
const learnSubtabLearn = document.getElementById('learnSubtabLearn');
const learnSubtabPlay = document.getElementById('learnSubtabPlay');
const learnPaneLearn = document.getElementById('learnPaneLearn');
const learnPanePlay = document.getElementById('learnPanePlay');

function isChessCourse(c) {
  return !!c && /chess/i.test(`${c.prompt || ''} ${c.title || ''}`);
}
function showLearnPane(which) {
  const play = which === 'play';
  learnSubtabLearn.classList.toggle('active', !play);
  learnSubtabPlay.classList.toggle('active', play);
  learnPaneLearn.style.display = play ? 'none' : '';
  learnPanePlay.style.display = play ? '' : 'none';
}
function updateLearnSubtabs() {
  const chess = isChessCourse(activeCourse);
  learnSubtabs.style.display = chess ? '' : 'none';
  if (!chess) showLearnPane('learn'); // never leave a non-chess course on the Play pane
}
learnSubtabLearn.addEventListener('click', () => showLearnPane('learn'));
learnSubtabPlay.addEventListener('click', () => showLearnPane('play'));
...
// "Chess Lessons (NEW)" shortcut — fills the topic and runs the exact same
// Create Course flow as typing "Chess" by hand (so the banked-day block and
// course-limit guard inside the submit handler still apply).
chessLessonsBtn.addEventListener('click', () => {
  createCourseInput.value = 'Chess';
  createCourseSubmitBtn.click();
});
...
// Lets the chess module (index.html) award XP/streak the exact same way
// every other learning game does, without duplicating the Firestore writes.
export { awardGameXp, bumpStreak };
...
async function generateLessonData(courseId, courseTitle, courseDescription, unit, unitIndex, lessonIndex, pregeneratedTitle, opts = {}) {
  const isReview = lessonIndex === LESSONS_PER_UNIT - 1;
  // opts.previousTopics: the background job tracks topics in memory instead of
  // re-reading every lesson doc in the course before each lesson.
  // opts.difficulty: the background job passes null — lessons generated ahead of
  // time can't react to a score the learner hasn't earned yet.
  const previousTopics = opts.previousTopics || await collectPreviousTopics(courseId, { unitIndex });

  const enableVisuals = getDiagramsEnabledFlag();
  const difficulty = 'difficulty' in opts
    ? opts.difficulty
    : computeDifficultySignal(activeCourse?.id === courseId ? activeCourse : null);
...
// ============================================================
// BACKGROUND LESSON GENERATION ("Generate Lessons" button)
// There's no server to run this while the app is closed, so it runs while
// the app is open: one lesson at a time, in order, from the learner's
// current lesson forward. It pauses the moment the app is backgrounded
// (visibilitychange + Capacitor's appStateChange) and picks back up when
// they return, unless they paused it themselves.
//
// Lessons are generated strictly in sequence on purpose: each lesson is told
// which topics earlier lessons in its unit already covered so it doesn't
// reteach them, and the unit review (15th lesson) needs the other 14 first.
// Topics are tracked in memory here instead of re-reading every lesson doc
// in the course before each lesson (what collectPreviousTopics() does).
//
// job = {
//   uid, courseId, courseTitle, courseDescription, units,
//   queue: [{ unitIndex, lessonIndex, pregeneratedTitle }], total, done,
//   paused, pauseReason: 'user' | 'leave' | 'error' | null,
//   cancelled, running, finished, failures, topicsByUnit: Map
// }
// ============================================================
const LESSON_GEN_MAX_FAILURES = 3; // consecutive failures before it pauses and asks the learner to retry
const LESSON_GEN_MAX_RATE_LIMIT_HITS = 3; // consecutive rate-limit rounds before it pauses (each round already includes the worker's own ~20s key-rotation wait)
const LESSON_GEN_RATE_LIMIT_WAIT_MS = 30000;
const RATE_LIMIT_RE = /429|rate.?limit|tokens per|\bTPD\b|\bTPM\b/i;

// Waits up to `ms`, but wakes early if the job is paused or cancelled.
async function waitWhileRunning(job, ms) {
  for (let waited = 0; waited < ms && !job.cancelled && !job.paused; waited += 500) {
    await new Promise((r) => setTimeout(r, 500));
  }
}

// One read of the course's lesson docs: which are already generated, plus
// their titles/topics (topics seed the in-memory previous-topics list).
async function scanCourseLessons(courseId) {
  const snap = await getDocs(collection(db, 'users', uid(), 'learnCourses', courseId, 'lessons'));
  const lessons = new Map();
  snap.forEach((d) => {
    if (!/^\d+_\d+$/.test(d.id)) return; // skips 'courseReview'
    const data = d.data();
    lessons.set(d.id, {
      hasParts: Array.isArray(data.parts),
      title: data.lessonTitle || null,
      topics: Array.isArray(data.topicsCovered) ? data.topicsCovered : [],
    });
  });
  return lessons;
}

// Every not-yet-generated lesson from the learner's current lesson through
// the last unit in scope (scopeUnits = how many units, counting the current one).
function buildLessonGenQueue(course, lessons, scopeUnits) {
  const queue = [];
  const firstUnit = course.currentUnitIndex || 0;
  const lastUnit = Math.min(UNITS_PER_COURSE - 1, firstUnit + scopeUnits - 1);
  for (let u = firstUnit; u <= lastUnit; u++) {
    if (!course.units?.[u]) continue;
    const startLesson = u === firstUnit ? (course.currentLessonIndex || 0) : 0;
    for (let l = startLesson; l < LESSONS_PER_UNIT; l++) {
      const info = lessons.get(`${u}_${l}`);
      if (info?.hasParts) continue;
      if (getLocalLesson(course.id, u, l)?.parts) continue; // saved locally, Firestore write still catching up
      queue.push({
        unitIndex: u,
        lessonIndex: l,
        pregeneratedTitle: l === LESSONS_PER_UNIT - 1 ? null : (info?.title || null),
      });
    }
  }
  return queue;
...
let lessonGenScan = null; // { courseId, lessons } — from the last time the modal was opened

function updateLessonGenBtn() {
  if (!lessonGenBtn) return;
  const c = activeCourse;
  const eligible = !!c && c.status !== 'generating' && (c.currentUnitIndex || 0) < UNITS_PER_COURSE;
  const busy = !!lessonGenJob && !lessonGenJob.finished && lessonGenJob.courseId === c?.id;
  lessonGenBtn.style.display = eligible && !busy ? 'flex' : 'none';
}

async function openLessonGenModal() {
  const c = activeCourse;
  if (!c || !uid()) return;
  lessonGenOptions.forEach((o) => { o.el.style.display = 'none'; });
  lessonGenModalSub.textContent = 'Checking which lessons are already generated…';
  lessonGenModalOverlay.classList.add('show');

  try {
    const lessons = await scanCourseLessons(c.id);
    if (!lessonGenModalOverlay.classList.contains('show') || activeCourse?.id !== c.id) return;
    lessonGenScan = { courseId: c.id, lessons };

    const counts = lessonGenOptions.map((o) => buildLessonGenQueue(c, lessons, o.scopeUnits).length);
    if (!counts[2]) {
      lessonGenModalSub.textContent = 'Every lesson in this course is already generated.';
      return;
    }
    lessonGenModalSub.textContent = 'Get lessons ready ahead of time so they open instantly. Generation runs while the app is open.';
    lessonGenOptions.forEach((o, i) => {
      // Hide a wider option when it would generate the same lessons as the narrower one before it.
      if (!counts[i] || (i > 0 && counts[i] === counts[i - 1])) return;
      o.el.innerHTML = `${o.label}<div class="kll-modal-btn-sub">${counts[i]} lesson${counts[i] === 1 ? '' : 's'}</div>`;
      o.el.style.display = '';
    });
  } catch (err) {
    console.error('Could not check lesson status:', err);
    lessonGenModalSub.textContent = "Couldn't check your lessons. Close this and try again.";
  }
}

lessonGenBtn.addEventListener('click', openLessonGenModal);
lessonGenModalCancelBtn.addEventListener('click', () => lessonGenModalOverlay.classList.remove('show'));
lessonGenOptions.forEach((o) => o.el.addEventListener('click', () => startLessonGenJob(o.scopeUnits)));

function startLessonGenJob(scopeUnits) {
  const c = activeCourse;
  const u = uid();
  if (!c || !u || !lessonGenScan || lessonGenScan.courseId !== c.id) return;
  const queue = buildLessonGenQueue(c, lessonGenScan.lessons, scopeUnits);
  lessonGenModalOverlay.classList.remove('show');
  if (!queue.length) return;

  cancelLessonGenJob(); // one job at a time

  const topicsByUnit = new Map();
  lessonGenScan.lessons.forEach((info, key) => {
    const unitIndex = Number(key.split('_')[0]);
    if (!topicsByUnit.has(unitIndex)) topicsByUnit.set(unitIndex, []);
    topicsByUnit.get(unitIndex).push(...info.topics);
  });

  lessonGenJob = {
    uid: u, courseId: c.id, courseTitle: c.title, courseDescription: c.description, units: c.units,
    queue, total: queue.length, done: 0,
    paused: false, pauseReason: null, cancelled: false, running: false, finished: false, failures: 0,
    rateLimitHits: 0, rateLimited: false,
    topicsByUnit,
  };
  renderLessonGenBar();
  updateLessonGenBtn();
  runLessonGenJob(lessonGenJob);
}

async function runLessonGenJob(job) {
  if (job.running) return; // the loop already running will pick up an un-pause
  job.running = true;
  try {
    while (!job.cancelled && !job.paused && job.queue.length) {
      if (uid() !== job.uid) { cancelLessonGenJob(); return; } // signed out / switched accounts

      const item = job.queue[0];
      const key = `${item.unitIndex}_${item.lessonIndex}`;
      const genKey = `${job.courseId}_${key}`;
      const advance = () => { job.queue.shift(); job.done++; renderLessonGenBar(); };

      // A title pre-generation for this lesson may be mid-flight (learner has
      // its Start modal open) — let it land so the two writes can't collide.
      if (job.courseId === activeCourse?.id && titleFetchPromises.has(key)) {
        await titleFetchPromises.get(key);
        if (job.cancelled) break;
      }
      // The learner is generating this exact lesson via Start right now.
      if (interactiveGenKeys.has(genKey)) { advance(); continue; }

      const lessonRef = doc(db, 'users', job.uid, 'learnCourses', job.courseId, 'lessons', key);
      const snap = await getDoc(lessonRef);
      if (job.cancelled) break;
      if ((snap.exists() && Array.isArray(snap.data().parts)) || getLocalLesson(job.courseId, item.unitIndex, item.lessonIndex)?.parts) {
        advance(); // already generated some other way since the scan
        continue;
      }
      const isReview = item.lessonIndex === LESSONS_PER_UNIT - 1;
      const pregeneratedTitle = isReview ? null : ((snap.exists() && snap.data().lessonTitle) || item.pregeneratedTitle || null);

      const work = (async () => {
        const data = await generateLessonData(
          job.courseId, job.courseTitle, job.courseDescription, job.units[item.unitIndex],
          item.unitIndex, item.lessonIndex, pregeneratedTitle,
          { previousTopics: [...(job.topicsByUnit.get(item.unitIndex) || [])], difficulty: null }
        );
        data.generatedAhead = true;
        // The learner may have generated it via Start while this was running;
        // never overwrite a lesson that may already be in progress or completed.
        const latest = await getDoc(lessonRef);
        if (latest.exists() && Array.isArray(latest.data().parts)) return null;
        saveLessonLocally(job.courseId, item.unitIndex, item.lessonIndex, data);
        await setDoc(lessonRef, data);
        clearLocalLesson(job.courseId, item.unitIndex, item.lessonIndex);
        return data;
      })();
      lessonGenInFlight.set(genKey, work);
      ...
      try {
        ...
        if (job.cancelled || job.pauseReason === 'leave') break; // app was backgrounded mid-request, retry on return

        // The worker already rotates through every Groq key and waits ~20s
        // before giving up, so a 429 reaching here means every key is limited
        // right now. That's not a "broken" failure: wait it out instead of
        // burning strikes. A per-minute limit clears in a bit; a per-day limit
        // won't, so after a few rounds pause and let the learner retry later.
        if (RATE_LIMIT_RE.test(String(err?.message))) {
          job.rateLimitHits++;
          if (job.rateLimitHits >= LESSON_GEN_MAX_RATE_LIMIT_HITS) {
            job.rateLimited = false;
            job.paused = true;
            job.pauseReason = 'ratelimit';
            renderLessonGenBar();
            break;
          }
          job.rateLimited = true;
          renderLessonGenBar();
          await waitWhileRunning(job, LESSON_GEN_RATE_LIMIT_WAIT_MS);
          job.rateLimited = false;
          renderLessonGenBar();
          continue;
        }
        ...
        job.failures++;
        if (job.failures >= LESSON_GEN_MAX_FAILURES) {
          job.paused = true;
          job.pauseReason = 'error';
          renderLessonGenBar();
          break;
        }
        await new Promise((r) => setTimeout(r, 2000 * job.failures));
        continue;
      } finally {
        lessonGenInFlight.delete(genKey);
      }

      if (data && Array.isArray(data.topicsCovered)) {
        if (!job.topicsByUnit.has(item.unitIndex)) job.topicsByUnit.set(item.unitIndex, []);
        job.topicsByUnit.get(item.unitIndex).push(...data.topicsCovered);
      }
      advance();
    }
  } finally {
  ...
  if (!job.cancelled && !job.queue.length) finishLessonGenJob(job);
}

function finishLessonGenJob(job) {
  job.finished = true;
  renderLessonGenBar();
  updateLessonGenBtn();
  setTimeout(() => {
    if (lessonGenJob === job) { lessonGenJob = null; renderLessonGenBar(); updateLessonGenBtn(); }
  }, 4000);
}

function cancelLessonGenJob() {
  if (lessonGenJob) lessonGenJob.cancelled = true; // any lesson mid-request still finishes and saves
  lessonGenJob = null;
  renderLessonGenBar();
  updateLessonGenBtn();
}

function renderLessonGenBar() {
  if (!lessonGenBar) return;
  const j = lessonGenJob;
  if (!j || j.cancelled) { lessonGenBar.classList.remove('show'); return; }

  lessonGenBar.classList.add('show');
  lessonGenBarFill.style.width = `${j.total ? Math.round((j.done / j.total) * 100) : 0}%`;
  const count = `${j.done} out of ${j.total} Lessons Generated`;

  if (j.finished) {
    lessonGenBarTitle.textContent = 'Lessons Generated';
    lessonGenBarSub.textContent = count;
    lessonGenBarIcon.textContent = 'check_circle';
    lessonGenBarToggle.style.display = 'none';
    return;
  }
  lessonGenBarTitle.textContent = 'Generating Lessons';
  lessonGenBarIcon.textContent = 'auto_stories';
  lessonGenBarToggle.style.display = '';
  const showPlay = j.paused && j.pauseReason !== 'leave';
  lessonGenBarToggleIcon.textContent = showPlay ? 'play_arrow' : 'pause';
  if (j.paused && j.pauseReason === 'ratelimit') lessonGenBarSub.textContent = 'Rate limited. Tap play to try again later.';
  else if (!j.paused && j.rateLimited) lessonGenBarSub.textContent = 'Rate limited, retrying shortly';
  else if (j.paused && j.pauseReason === 'error') lessonGenBarSub.textContent = "Couldn't reach the server. Tap play to retry.";
  else if (j.paused && j.pauseReason === 'user') lessonGenBarSub.textContent = `Paused: ${count}`;
  else lessonGenBarSub.textContent = count;
}

lessonGenBarToggle.addEventListener('click', () => {
  const j = lessonGenJob;
  if (!j || j.finished) return;
  if (j.paused) {
    j.paused = false; j.pauseReason = null; j.failures = 0; j.rateLimitHits = 0;
    renderLessonGenBar();
    runLessonGenJob(j);
  } else {
    j.paused = true; j.pauseReason = 'user';
    renderLessonGenBar();
  }
});
lessonGenBarClose.addEventListener('click', cancelLessonGenJob);

// Pause when the app is backgrounded, resume when it's back (unless the
// learner paused it themselves). Both the web event and Capacitor's native
// app-state event are wired since either can fire first inside the iOS webview.
function onLessonGenAppLeave() {
  const j = lessonGenJob;
  if (j && !j.finished && !j.paused) { j.paused = true; j.pauseReason = 'leave'; renderLessonGenBar(); }
}
function onLessonGenAppReturn() {
  const j = lessonGenJob;
  if (j && !j.finished && j.paused && j.pauseReason === 'leave') {
    j.paused = false; j.pauseReason = null;
    renderLessonGenBar();
    runLessonGenJob(j);
  }
}
document.addEventListener('visibilitychange', () => (document.hidden ? onLessonGenAppLeave() : onLessonGenAppReturn()));
try {
  window.Capacitor?.Plugins?.App?.addListener?.('appStateChange', ({ isActive }) => (isActive ? onLessonGenAppReturn() : onLessonGenAppLeave()));
} catch (err) { console.warn('Could not hook Capacitor appStateChange for lesson generation:', err); }
...
function renderLessonDifficultyBadge(isReview, isPrevious, generatedAhead = false) {
  ...
  // Lessons generated ahead of time were written before the learner's next
  // score existed, so an Easier/Harder/Next badge would promise something the
  // lesson doesn't do.
  if (generatedAhead && !isPrevious) {
...
async function openLessonStartModal(unitIndex, lessonIndex) {
  ...
  // Unit Reviews have a fixed title; every other lesson gets a spinner in
  // place of Start until its title exists.
  setLessonStartGate(isReview ? 'ready' : 'loading');
  ...
  if (data) {
    ...
    renderLessonDifficultyBadge(isReview, isPreviousLesson, !!data.generatedAhead);
  }

  // Start stays hidden until there's a real title (or the lesson already has
  // its content, in which case it's obviously ready).
  const needsTitle = !isReview && !(data && (data.lessonTitle || Array.isArray(data.parts)));
  if (needsTitle) {
    lessonStartSub.textContent = "You haven't started this lesson yet.";
    loadLessonStartTitle(unitIndex, lessonIndex);
  } else {
    setLessonStartGate('ready');
  }
}

// Swaps the Start button for a spinner (title still generating) or a Try
// again button (title generation failed) — Start only shows once the lesson
// title exists, so a lesson can never be generated before its title.
function setLessonStartGate(state) {
  lessonStartGateState = state;
  lessonStartBtn.style.display = state === 'ready' ? '' : 'none';
  lessonStartSpinner.classList.toggle('show', state === 'loading');
  lessonStartRetryBtn.style.display = state === 'failed' ? '' : 'none';
}

// Kicks off the title-only generation (a lightweight call — full content
// still waits for Start) and flips the gate to 'ready' once the title lands.
function loadLessonStartTitle(unitIndex, lessonIndex) {
  setLessonStartGate('loading');
  fetchAndSaveLessonTitle(unitIndex, lessonIndex).then((title) => {
    // Bail if the modal was closed or a different lesson was opened meanwhile.
    if (!pendingLessonRef || pendingLessonRef.unitIndex !== unitIndex || pendingLessonRef.lessonIndex !== lessonIndex) return;
    if (title) {
      lessonStartTitle.textContent = title;
      setLessonStartGate('ready');
    } else {
      lessonStartTitle.textContent = 'Lesson';
      lessonStartSub.textContent = "Couldn't load the lesson title.";
      setLessonStartGate('failed');
    }
  });
}

lessonStartRetryBtn.addEventListener('click', () => {
  if (!pendingLessonRef) return;
  lessonStartTitle.textContent = 'Loading Lesson Title...';
  lessonStartSub.textContent = "You haven't started this lesson yet.";
  loadLessonStartTitle(pendingLessonRef.unitIndex, pendingLessonRef.lessonIndex);
});
...
lessonStartBtn.addEventListener('click', async () => {
  if (!pendingLessonRef) return;
  if (lessonStartGateState !== 'ready') return; // title not generated yet
  ...
  // Same idea for the background "Generate Lessons" job: if it's writing this
  // exact lesson right now, wait for it instead of generating a second copy.
  const genKey = `${activeCourse.id}_${key}`;
  const bgInFlight = lessonGenInFlight.get(genKey);
  if (bgInFlight) {
    await bgInFlight.catch(() => {});
    if (genToken.cancelled) return;
  }
  ...
  if (snap.exists() && Array.isArray(snap.data().parts)) {
    ...
    interactiveGenKeys.add(genKey); // tells the background job to skip this lesson
    try {
      const unit = activeCourse.units[unitIndex];
      ...
      if (genToken.cancelled) { interactiveGenKeys.delete(genKey); stopLessonProgressBar(); return; }
      ...
      interactiveGenKeys.delete(genKey);
      stopLessonProgressBar();
      if (genToken.cancelled) return;
      lessonStartBtn.disabled = false;
      lessonStartBtnLabel.textContent = 'Start';
...
// gameHistory.js) so the worker's AI prompt can avoid recent repeats.
//
// The worker's AI is only *asked* to avoid history, and models routinely
// ignore that (a topic tends to pull the same "best" 5-letter word every
// time). So the result is checked here: a word already in history is
// rejected and re-requested, with the rejected word added to the history sent
// along so the prompt sees it as just played. If every attempt repeats, the
// one played longest ago is used.
const WORDGRID_MAX_ATTEMPTS = 4;
async function fetchWordGridWord(topic, history = []) {
  const seen = new Map(); // WORD -> daysAgo, for everything played (or already rejected this call)
  history.forEach((h) => seen.set(String(h.text).trim().toUpperCase(), h.daysAgo ?? 0));
  const sentHistory = [...history];
  const repeats = [];

  for (let attempt = 0; attempt < WORDGRID_MAX_ATTEMPTS; attempt++) {
    const res = await fetch(LEARN_WORKER_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'generateWordGridWord', topic, history: sentHistory }),
    });
    const data = await res.json();
    ...
    if (!seen.has(data.word)) return data;

    repeats.push({ data, daysAgo: seen.get(data.word) });
    sentHistory.unshift({ text: data.word, daysAgo: 0 });
  }

  console.warn('Word Grid: every attempt repeated a played word — using the one played longest ago.');
  repeats.sort((a, b) => b.daysAgo - a.daysAgo);
  return repeats[0].data;
}
